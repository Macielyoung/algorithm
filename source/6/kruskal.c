...
{  
    Graph p = g;  
    int count = 0;  
    int Vernum = strlen(p.ver);  
    for( int i=0; i<Vernum; i++ )  
        for( int j=i; j<Vernum; j++ )     //邻接矩阵对称，计算上三角元素和即可  
            if( 0 != p.edg[i][j] )  
                count++;  
    return count;  
}  
...
{  
    ...
    for( i=0; i<Vernum; i++ )             //边集数组初始化,同样只考虑上三角矩阵  
        for( j=i; j<Vernum; j++ )  
            if( 0 != g.edg[i][j] ) {  
                p[k].begin = i;  
                p[k].end = j;  
...
{  
    ...
    int *index = (int *)malloc(sizeof(int)*Vernum);          //index数组，其元素为连通分量的编号，index[i] = index[j] 表示编号为i 和 j的顶点在同一个连通分量中，反之则不在同一个连通分量  
    int *MSTEdge = (int *)malloc(sizeof(int)*Vernum-1);      //MSTEdge数组，用来存储已确定的MST的边的编号，共Vernum-1条边  
    int k= 0;  
    int WeightSum = 0;  
    int IndexBegin, IndexEnd;  
  
    for(int i=0; i<Vernum; i++ )      //初始化所有index = -1  
        index[i] = -1;  
        ...
        for(int j=0; j<EdgNum; j++ ) {  
            if( !(index[p[j].begin]>=0 && index[p[j].end]>=0 && index[p[j].begin]==index[p[j].end]) ) {  //找到当前还没加入到同一个连通分量且权值最下的边  
                ...
                if( (-1 == index[p[j].begin]) && (-1 == index[p[j].end]) )          //该边的两个顶点都没加入过任何一个连通分量  
                    ...
                    IndexEnd = index[p[j].end];  
                    IndexBegin = index[p[j].begin];  
                    for(int n=0; n<Vernum; n++ )                              //该边的两个顶点都已经存在于两个不同的连通分量中  
